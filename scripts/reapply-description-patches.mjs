...
for (const [label, file] of PATCHES) {
  ...
  const apply = spawnSync(process.execPath, [script], { stdio: 'pipe', encoding: 'utf8' });
  const check = spawnSync(process.execPath, [script, '--check'], { stdio: 'pipe', encoding: 'utf8' });
  const ok = apply.status === 0 && check.status === 0;
  results.push({ label, ok, applyStatus: apply.status, checkStatus: check.status, detail: (apply.stderr || apply.stdout || '').trim() });
}

for (const r of results) {
  console.log((r.ok ? '✓' : '✗') + ' ' + r.label.padEnd(20) + ' apply=' + (r.applyStatus ?? 'killed') + ' check=' + (r.checkStatus ?? 'killed'));
  if (!r.ok && r.detail) {

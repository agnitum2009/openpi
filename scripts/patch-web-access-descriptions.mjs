...
const ROOT = join(process.env.HOME ?? '', '.pi/agent/npm/node_modules/pi-web-access');
...
const SOURCE_CHECK_DESCRIPTION = "Claim check against web sources: bounded machine-readable evidence with passage citations.";
...
const GET_SEARCH_CONTENT_STATIC = "Retrieve slices / matching passages in a previous ";
const GET_SEARCH_CONTENT_INTERP = '${storedContentSources}';

// promptSnippet compressions (snippets are not behavioral guidelines).
const SNIPPETS = {
  webSearch: "Web research. Prefer {queries:[...]} with 2-4 varied angles; omit provider unless overriding the configured default.",
  fetchContent: "Fetch readable/raw URL content, images, GitHub repos, videos; mode answer uses only the fetched source.",
  sourceCheck: "Verify a claim with structured source evidence and passage citations.",
};
...
function assertPlain(text, label) {
  if (text.includes(String.fromCharCode(36) + '{') || text.includes(String.fromCharCode(96))) {
    throw new Error('NEW text for ' + label + ' contains ${} or a backtick — keep it plain text.');
  }
}
...
try {
  ...
  for (const [label, text] of Object.entries(SNIPPETS)) assertPlain(text, 'snippet ' + label);
  ...
  for (const target of TARGETS) {
    ...
    const ws = locateField(source, 'name: toolNames.webSearch', 'description', 400);
    anchors.push({ label: 'web_search description', found: ws, text: WEB_SEARCH_DESCRIPTION, interp: null });
    // source_check description (full replace).
    const sc = locateField(source, 'name: toolNames.sourceCheck', 'description', 400);
    anchors.push({ label: 'source_check description', found: sc, text: SOURCE_CHECK_DESCRIPTION, interp: null });
    // fetch_content description (static prefix, keep interpolation).
    const fc = locateField(source, 'name: toolNames.fetchContent', 'description', 400);
    if (fc && fc.quote === '`') {
      const interpAt = source.indexOf(FETCH_CONTENT_INTERP, fc.start);
      if (interpAt >= 0 && interpAt < (fc.end ?? Infinity)) fc.end = interpAt;
    }
    anchors.push({ label: 'fetch_content description', found: fc, text: FETCH_CONTENT_STATIC, interp: FETCH_CONTENT_INTERP });
    // get_search_content description (static prefix, keep interpolation).
    const gc = locateField(source, 'name: toolNames.getSearchContent', 'description', 400);
    if (gc && gc.quote === '`') {
      const interpAt = source.indexOf(GET_SEARCH_CONTENT_INTERP, gc.start);
      if (interpAt >= 0 && interpAt < (gc.end ?? Infinity)) gc.end = interpAt;
    }
    anchors.push({ label: 'get_search_content description', found: gc, text: GET_SEARCH_CONTENT_STATIC, interp: GET_SEARCH_CONTENT_INTERP });
    // snippets (web_search / fetch_content / source_check).
    anchors.push({ label: 'web_search snippet', found: locateField(source, 'name: toolNames.webSearch', 'promptSnippet', 3000), text: SNIPPETS.webSearch, interp: null });
    anchors.push({ label: 'fetch_content snippet', found: locateField(source, 'name: toolNames.fetchContent', 'promptSnippet', 3000), text: SNIPPETS.fetchContent, interp: null });
    anchors.push({ label: 'source_check snippet', found: locateField(source, 'name: toolNames.sourceCheck', 'promptSnippet', 3000), text: SNIPPETS.sourceCheck, interp: null });

    const missing = anchors.filter((a) => !a.found);
    if (missing.length > 0) {
      console.error('[!] not located: ' + missing.map((m) => m.label).join(', ') + ' — layout changed; re-derive manually.');
      allApplied = false;
      continue;
    }

    const blocksApplied = anchors.every((a) => source.slice(a.found.start, a.found.end) === renderText(a.text, a.found.quote));
    ...
    const edits = anchors.filter((a) => a.found).sort((a, b) => b.found.start - a.found.start);
    let next = source;
    for (const { label, found, text, interp } of edits) {
      const rawBefore = source.slice(found.start, found.end);
      const before = found.quote === '"' ? unescapeJson(rawBefore).length : rawBefore.length;
      ...
      console.log('  [' + basename + '] ' + label.padEnd(28) + ' ' + String(before).padStart(5) + ' -> ' + String(after).padStart(5) + ' chars (-' + (before - after) + ', -' + Math.round(((before - after) / before) * 100) + '%)');
    }
  ...
  if (!anyLocated) {
    console.error('No pi-web-access target files found — check the extension install path.');
    process.exit(1);
  }
  if (check) {
    if (allApplied) { console.log('Patch already applied ✓'); process.exit(0); }
    console.error('Patch NOT applied — run: node scripts/patch-web-access-descriptions.mjs');
    process.exit(1);
  }
  if (!allApplied && !changed) {
    console.error('Some text blocks were not located — the extension layout changed; re-derive manually.');
    process.exit(1);
  }
  console.log('pi-web-access descriptions patched ✓ (backup: index.ts.bak-tool-descriptions)');
} catch (error) {

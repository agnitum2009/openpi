...
const ROOT = join(
  process.env.HOME ?? "",
  ".pi/agent/npm/node_modules/pi-hermes-memory",
);
const TARGETS = [
...
const TEXT_REPLACEMENTS = [
  {
    file: "src/tools/memory-tool.ts",
    label: "common suffix",
    from: "This action-specific tool accepts only the parameters listed in its schema.",
    to: "Accepts only the parameters listed in its schema.",
  },
  {
    file: "src/tools/memory-tool.ts",
    label: "memory_add action line",
    from: "Add one durable entry. The target and content fields are required.",
    to: "Add one durable entry.",
  },
  {
    file: "src/tools/memory-tool.ts",
    label: "memory_replace action line",
    from: "Replace one existing entry. The target, old_text, and content fields are required.",
    to: "Replace one existing entry.",
  },
  {
    file: "src/tools/memory-tool.ts",
    label: "memory_remove action line",
    from: "Remove one existing entry. The target and old_text fields are required.",
    to: "Remove one existing entry.",
  },
];
...
function collectAnchors(target) {
  ...
  if (basename === "constants.ts") {
    return [
      {
        label: "MEMORY_TOOL_DESCRIPTION",
        locate: (s) => locateConst(s, "MEMORY_TOOL_DESCRIPTION"),
        text: DESCRIPTIONS.memory_tool_description,
      },
      {
        label: "SKILL_TOOL_DESCRIPTION",
        locate: (s) => locateConst(s, "SKILL_TOOL_DESCRIPTION"),
        text: DESCRIPTIONS.skill_tool_description,
      },
    ];
  }
  if (basename === "memory-search-tool.ts") {
    return [
      {
        label: "memory_search",
        locate: locateMemorySearch,
        text: DESCRIPTIONS.memory_search,
      },
    ];
  }
  if (basename === "session-search-tool.ts") {
    return [
      {
        label: "session_search (anchors)",
        locate: (s) => locateRegisterToolDescription(s, 0),
        text: DESCRIPTIONS.session_search_anchors,
      },
      {
        label: "session_search (legacy)",
        locate: (s) => locateRegisterToolDescription(s, 1),
        text: DESCRIPTIONS.session_search_legacy,
      },
    ];
  }
  return [];
...
try {
  ...
  for (const target of TARGETS) {
    ...
    const repStates = TEXT_REPLACEMENTS.filter(
      (r) => r.file === "src/tools/" + basename,
    ).map((r) => ({ ...r, applied: source.includes(r.to) }));
    for (const r of repStates) if (!r.applied) allApplied = false;
    const missingRep = repStates.find(
      (r) => !r.applied && !source.includes(r.from),
    );
    if (missingRep) {
      console.error(
        "[!] " +
    ...
    const blocksApplied =
      ...
      located.every(
        (l) =>
          source.slice(l.found.start, l.found.end) ===
          renderDescription(l.text, l.found.quote),
      );
    ...
    for (const r of repStates) {
      ...
      console.log(
        "  [" +
          ...
          r.label.padEnd(26) +
          " " +
          String(r.from.length).padStart(5) +
          " -> " +
          String(r.to.length).padStart(5) +
          " chars (-" +
          (r.from.length - r.to.length) +
          ", -" +
          Math.round(((r.from.length - r.to.length) / r.from.length) * 100) +
          "%)",
      );
    }
    const edits = located
    ...
    for (const { label, found, text } of edits) {
      ...
      next =
        ...
        renderDescription(text, found.quote) +
  ...
  if (!anyLocated) {
    console.error(
      "No pi-hermes-memory target files found — check the extension install path.",
    );
    process.exit(1);
  }
  if (check) {
    ...
    console.error(
      "Patch NOT applied — run: node scripts/patch-hermes-memory-descriptions.mjs",
    );
    process.exit(1);
  }
  ...
  console.log(
    "pi-hermes-memory descriptions patched ✓ (backups: *.bak-tool-descriptions)",
  );
} catch (error) {
  console.error(

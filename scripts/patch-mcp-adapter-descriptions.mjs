...
const ROOT = join(process.env.HOME ?? '', '.pi/agent/npm/node_modules/pi-mcp-adapter');
const TARGETS = [join(ROOT, 'direct-tools.ts'), join(ROOT, 'index.ts')];

const PREFIX_TEXT = "MCP gateway — server status, tool search/describe, auth, and single MCP tool calls. For several MCP calls with logic between them use mcpScript; call non-MCP Pi tools directly, not through mcp.\\n";
...
try {
  ...
  for (const target of TARGETS) {
    ...
    if (basename === 'direct-tools.ts') {
      anchors.push({ label: 'mcp prefix', found: locatePrefix(source), text: PREFIX_TEXT, quote: '`' });
      anchors.push({ label: 'mcp usage block', found: locateUsageBlock(source), raw: USAGE_BLOCK });
    } else {
      anchors.push({ label: 'mcpScript description', found: locateMcpScript(source), text: MCPSCRIPT_TEXT, quote: '"' });
    }
    ...
    const blocksApplied = anchors.every((a) =>
      a.raw !== undefined
        ...
        : source.slice(a.found.start, a.found.end) === renderText(a.text, a.quote ?? '`'));
    ...
    for (const a of edits) {
      ...
      const replacement = a.raw !== undefined ? a.raw : renderText(a.text, a.quote ?? '`');
      const after = replacement.length;
      next = next.slice(0, a.found.start) + replacement + next.slice(a.found.end);
      console.log('  [' + basename + '] ' + a.label.padEnd(24) + ' ' + String(before).padStart(5) + ' -> ' + String(after).padStart(5) + ' chars (-' + (before - after) + ', -' + Math.round(((before - after) / before) * 100) + '%)');
    }
  ...
  if (!anyLocated) {
    console.error('No pi-mcp-adapter target files found — check the extension install path.');
    process.exit(1);
  }
  if (check) {
    if (allApplied) { console.log('Patch already applied ✓'); process.exit(0); }
    console.error('Patch NOT applied — run: node scripts/patch-mcp-adapter-descriptions.mjs');
    process.exit(1);
  }
  if (!allApplied && !changed) {
    console.error('Some blocks were not located — the extension layout changed; re-derive manually.');
    process.exit(1);
  }
  console.log('pi-mcp-adapter descriptions patched ✓ (backups: *.bak-tool-descriptions)');
} catch (error) {

...
if (!extPath || !toolName || !paramsJson) {
  console.error("usage: HOME=<temp> node ext-call.mjs <ext-entry> <toolName> <json-params>");
  process.exit(1);
}
...
runtime.setActiveTools = (names) => { activeTools = Array.isArray(names) ? [...names] : activeTools; };
runtime.refreshTools = () => {};
const { extensions, errors } = await loadExtensions([extPath], process.cwd(), undefined, runtime);
if (errors.length) {

...
function total(f) {
  return f.desc + f.params + f.snippet + f.guidelines;
}
...
function printTable(rows, grandLabel) {
  ...
  for (const r of rows) {
    t.desc += r.f.desc;
    t.params += r.f.params;
    t.snippet += r.f.snippet;
    t.guidelines += r.f.guidelines;
  }
  for (const r of rows) {
    console.log(
      "  " +
        r.name.padEnd(22) +
        " desc " +
        String(r.f.desc).padStart(6) +
        " params " +
        String(r.f.params).padStart(6) +
        " snippet " +
        String(r.f.snippet).padStart(4) +
        " guidelines " +
        String(r.f.guidelines).padStart(5) +
        " | ALL " +
        String(total(r.f)).padStart(6),
    );
  }
  const all = total(t);
  console.log(
...
if (mode === "--kernel") {
  const toolsMod = await import(
    "/home/umax/.local/lib/node_modules/@earendil-works/pi-coding-agent/dist/core/tools/index.js"
  );
...
if (mode === "--ext") {
  ...
  if (paths.length === 0) {
    console.error("no extension paths");
    process.exit(1);
  }
  ...
  for (let i = 0; i < paths.length; i++) {
    ...
    const rows = [];
    for (const [name, { definition: def }] of ext.tools)
      rows.push({ name, f: fieldSize(def) });
    if (rows.length === 0) {
    ...
    grand.desc += r.t.desc;
    grand.params += r.t.params;
    grand.snippet += r.t.snippet;
    grand.guidelines += r.t.guidelines;
    grandAll += r.all;
  }
  if (any) {
    console.log(

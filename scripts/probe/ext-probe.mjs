...
console.log(
  ...
  "params".padStart(8),
  "snippet".padStart(8),
  "guidelines".padStart(11),
);
for (const r of rows) {
  console.log(
    r.name.padEnd(18),
    String(r.desc).padStart(7),
    String(r.params).padStart(8),
    String(r.snippet).padStart(8),
    String(r.guidelines).padStart(11),
  );
}
console.log(
  `TOTALS: desc=${totalDesc} params=${totalParams} snippet=${totalSnippet} guidelines=${totalGuidelines} ALL=${total} (~${Math.round(total / 4)} tok) toolCount=${rows.length}`,
);
if (withText) {
  for (const [name, { definition: d }] of ext.tools) {
    console.log(`\n===== TOOL: ${name} =====`);

...
const BUNDLE = process.env.HOME + "/.pi/agent/npm/node_modules/context-mode/server.bundle.mjs";
    ...
    try {
      ...
      if (m.id === 1) send({ jsonrpc: "2.0", id: 2, method: "tools/list", params: {} });
        ...
        for (const t of tools) {
          ...
          console.log(t.name.padEnd(22) + " desc " + String((t.description ?? "").length).padStart(6) + " params " + String(JSON.stringify(t.inputSchema ?? {}).length).padStart(6));
        }
        console.log("TOTAL: desc " + desc + " params " + params + " ALL " + (desc + params) + " (~" + Math.round((desc + params) / 4) + " tok) tools=" + tools.length);
        child.kill(); process.exit(0);
      }
    } catch {}
  }
});
child.on("exit", (c) => { if (!buf) { console.error("exit", c); process.exit(1); } });
send({ jsonrpc: "2.0", id: 1, method: "initialize", params: { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "probe", version: "0" } } });
setTimeout(() => { child.kill(); process.exit(1); }, 20000);
